...
def on_click():
    ...
    if(form.lineEdit_2.text()=="" and form.lineEdit_3.text()==""):
        msg = QMessageBox()
        ...
        elif(form.comboBox.currentText() != "Перестановки без повторений"):#Вывод сообщения об ошибке
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Information)
            msg.setText("Некорректное значение n и/или m")
            msg.setWindowTitle("Ошибка")
        ...
        if ( n<= 100 and form.comboBox.currentText() == "Перестановки без повторений") :
            form.lineEdit.setPlainText(str(factorial(n)))
        elif((form.comboBox.currentText() != "Сочетания без повторений") and (form.comboBox.currentText() != "Размещения с повторениями")):#Вывод сообщения об ошибке
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Information)
